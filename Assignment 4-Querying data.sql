...
select CONCAT (fname,' ',lname) AS Name,country_name,rating from person where country_name='USA' or rating> 4.5;
...
select CONCAT (fname,' ',lname) AS Name,country_name from person where country_name in ('USA','Canada','UK');


---#8 FIND ALL PERSONS NOT FROM COUNTRIES INDIA AND AUSTRALIA
select CONCAT (fname,' ',lname) AS Name,country_name from person where country_name NOT IN('India','Australia');
